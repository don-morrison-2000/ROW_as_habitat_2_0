...
LOG_FILE = os.path.join(arcpy.env.scratchFolder, 'logs', 'row_log_%i.txt' % os.getpid())
...
def get(log_name):
    ...
    if not os.path.exists(LOG_FILE):
        for file in glob.glob(os.path.join(arcpy.env.scratchFolder, 'logs', 'row_log_*.txt')):

...
def __remove_obsolete_sde_objects(row_fgdb, item_spec):
    # Find sde object to keep (feature classes, tables, related tables)
    keep_objects = []
    for l_spec in [l for l in item_spec['layers'] if 'sde_source' in l.keys()]:
...
# def __upload_zip_files (gis, zip_files):
#     title = f"Feature Layer Archive {dt.now().strftime('%Y-%m-%d_%H-%M-%S')}.zip"
#     item_fgdb_zip = os.path.join(arcpy.env.scratchFolder, f"{title}.zip")
#     with zipfile.ZipFile(item_fgdb_zip, 'w') as zipMe:
...
#     } 
#     job = gis.content.folders.get(c.MODEL_ORG_ID).add(item_properties=item_properties, file=item_fgdb_zip)
#     map(os.remove, [item_fgdb_zip] + zip_files)
#     return

# def __publish_fgdb (gis, fgdb, item_spec):
#     #item_fgdb_zip = os.path.join(arcpy.env.scratchFolder, f"item_fgdb_{item_spec['id']}.zip")
#     item_fgdb_zip = os.path.join(arcpy.env.scratchFolder, "item_fgdb.zip")
#     if os.path.exists(item_fgdb_zip):
#         os.remove(item_fgdb_zip)
#     logger.debug (f"Zipping {fgdb} to {item_fgdb_zip}")

#     shutil.make_archive(os.path.splitext(item_fgdb_zip)[0], 'zip', root_dir=os.path.dirname(fgdb), base_dir=os.path.basename(fgdb))
#     item_properties = {
#         "title": item_spec['title'],
#         "description": item_spec['description'].replace('{ORG_NAME}', c.MODEL_ORG_NAME),
#         "tags": "",
#         "type": "File Geodatabase", 
#         "snippet": item_spec['snippet'].replace('{ORG_NAME}', c.MODEL_ORG_NAME)
#     }    

#     logger.debug (f"Uploading {item_fgdb_zip} to ArcGIS Online")
#     job = gis.content.folders.get(c.MODEL_ORG_ID).add(item_properties=item_properties, file=item_fgdb_zip)
#     fgdb_item = __wait_for_job(job)

#     logger.debug (f"Delete existing published item")
#     try:
#         existing_published_item = [i for i in gis.users.me.items(folder=c.MODEL_ORG_ID, max_items=-1) if i.title == item_spec['title'] and  i.type == item_spec['type']][0]
#         gis.content.delete_items([existing_published_item])
#     except:
#         pass

#     logger.debug(f"Publishing item {fgdb_item.id} as a Feature Service")
#     publish_parameters=gis.content.analyze(item=fgdb_item, file_type='fileGeodatabase')['publishParameters']
#     #publish_parameters['name'] = str(random.randint(10000000, 99999999))
#     publish_parameters['name'] = f"{c.MODEL_ORG_ID}.{item_spec['id']}.{random.randint(10000000, 99999999)}"
#     published_service = fgdb_item.publish(publish_parameters=publish_parameters, file_type='fileGeodatabase', overwrite=False)

#     logger.debug(f"Deleting the uploaded zip file")
#     gis.content.delete_items([fgdb_item])
#     return published_service


# def __publish_fgdb (gis, fgdb, item_spec):
#     # logger.debug (f"Delete existing published item")
#     # for i in [i for i in gis.users.me.items(folder=c.MODEL_ORG_ID, max_items=1000) if i.title == item_spec['title'] and  i.type in [item_spec['type'], 'Service Definition']]:
...
#     aprx = arcpy.mp.ArcGISProject(os.path.join(AppInfo.path_code_base, 'config', 'templates', 'blank_project.aprx'))
...
#     for layer_spec in [l for l in item_spec['layers'] if len(l['sde_source']) > 0]:
...
#     aprxMap = aprx.listMaps()[0] 
#     #service_name = f"{c.MODEL_ORG_ID}__{item_spec['id']}__{random.randint(10000000, 99999999)}"
#     service_name = f"{c.MODEL_ORG_ID}__{item_spec['id']}"
...
#     sharing_draft.summary = item_spec['snippet'].replace('{ORG_NAME}', c.MODEL_ORG_NAME)
#     sharing_draft.tags = f"{item_spec['id']},{c.MODEL_ORG_ID}"
#     sharing_draft.description = item_spec['description'].replace('{ORG_NAME}', c.MODEL_ORG_NAME)
...
#     logger.debug (f"Uploading the service item")
#     sd_item = gis.content.add({'title': service_name, 'tags': 'map, python, automated', 'type': 'Service Definition'}, sd_path)
#     logger.debug (f"Publishing the hosted feature layer")
#     published_item = sd_item.publish()
#     sd_item.update({'title': item_spec['title']})
#     sd_item.move(c.MODEL_ORG_ID)
#     published_item.update({'title': item_spec['title']})
#     published_item.move(c.MODEL_ORG_ID)
#     return


def __publish_fgdb (gis, fgdb, item_spec):
    service_name = item_spec['id']

    try:
        # If a sd has already been published, it has to be moved to root folder for the overwrite to work
        sd_item = [i for i in gis.users.me.items(folder=c.MODEL_ORG_ID, max_items=1000) if i.title==item_spec['title'] and i.type=='Service Definition'][0]
        sd_item.move('/')
    except Exception:
        pass
    ...
    logger.debug (f"Creating a map with the file geodatbase layers")
    aprx_path = os.path.join(arcpy.env.scratchFolder, 'blank_project.aprx')
    aprx = arcpy.mp.ArcGISProject(c.CODE_BASE, 'row', 'templates', 'blank_project.aprx')
    aprxMap = aprx.listMaps()[0]
    ...
    logger.debug (f"Creating a sharing draft")
    sharing_draft = aprxMap.getWebLayerSharingDraft("HOSTING_SERVER", "FEATURE", service_name)
    sharing_draft.summary = item_spec['snippet'].replace('{ORG_NAME}', c.MODEL_ORG_NAME).replace('{ORG_ID}', c.MODEL_ORG_ID)
    sharing_draft.tags = f"{c.CLONE_TAG_ROOT}{item_spec['id']},{c.MODEL_ORG_ID}"
    sharing_draft.description = item_spec['description'].replace('{ORG_NAME}', c.MODEL_ORG_NAME).replace('{ORG_ID}', c.MODEL_ORG_ID)
    sharing_draft.overwriteExistingService = True
    sharing_draft.exportToSDDraft(sddraft_path)
    ...
    sd_item.update({'title': item_spec['title'].replace('{ORG_NAME}', c.MODEL_ORG_NAME).replace('{ORG_ID}', c.MODEL_ORG_ID), 'tags': c.MODEL_ORG_ID})
    sd_item.move(c.MODEL_ORG_ID)
    return
...
def run():
    ...
    for x in range (1,2):
        ...
        for item_spec in [i for i in c.ITEM_SPECS if i['type'] == 'Feature Service']:
            ...
            logger.info (f"Processing {item_spec['title']}. Importing SDE objects") 
            row_fgdb = __import_sde_schema(export_xml_file)
        
            logger.info (f"Removing unnecessary SDE object from {row_fgdb}") 
            __remove_obsolete_sde_objects(row_fgdb, item_spec)

            arcpy.env.workspace = row_fgdb
            fcs += [os.path.join(row_fgdb, fc) for fc in arcpy.ListFeatureClasses() + arcpy.ListTables()]

            logger.info (f"Publishing {item_spec['title']}") 
            __publish_fgdb (gis, row_fgdb, item_spec)
